...
// src/types.ts
var ExitType, ExitStatus, CeremonyState, ContinuityProofType, CeremonyRole, SuccessorTrustLevel, StatusConfirmation, CoercionLabel, EXIT_CONTEXT_V1, EXIT_SPEC_VERSION;
var init_types = __esm({
  "src/types.ts"() {
    "use strict";
    ExitType = /* @__PURE__ */ ((ExitType2) => {
    ...
    ContinuityProofType = /* @__PURE__ */ ((ContinuityProofType2) => {
    ...
    CeremonyRole = /* @__PURE__ */ ((CeremonyRole2) => {
    ...
    SuccessorTrustLevel = /* @__PURE__ */ ((SuccessorTrustLevel2) => {
    ...
    StatusConfirmation = /* @__PURE__ */ ((StatusConfirmation2) => {
    ...
    CoercionLabel = /* @__PURE__ */ ((CoercionLabel3) => {
...
// src/errors.ts
var ExitError, ValidationError, SigningError, VerificationError, CeremonyError, StorageError;
var init_errors = __esm({
  "src/errors.ts"() {
    ...
    ValidationError = class extends ExitError {
      errors;
      /**
    ...
    SigningError = class extends ExitError {
      constructor(message) {
        super("SIGNING_FAILED", message);
        this.name = "SigningError";
      }
    };
    VerificationError = class extends ExitError {
      constructor(message) {
        super("VERIFICATION_FAILED", message);
        this.name = "VerificationError";
      }
    };
    CeremonyError = class extends ExitError {
      currentState;
      attemptedState;
      validTransitions;
      /**
    ...
    StorageError = class extends ExitError {
      constructor(message) {
...
function canonicalize(obj) {
  const result = (0, import_canonicalize.default)(obj);
...
function addModule(marker, key, module2) {
  const updated = { ...marker, [key]: module2 };
  updated.id = `urn:exit:${computeId(updated)}`;
  return updated;
}
var import_sha256, import_canonicalize, EMPTY_PROOF;
var init_marker = __esm({
  "src/marker.ts"() {
    "use strict";
    import_sha256 = require("@noble/hashes/sha256");
    import_canonicalize = __toESM(require("canonicalize"), 1);
    init_types();
    init_errors();
    EMPTY_PROOF = {
...
// src/git-ledger.ts
var git_ledger_exports = {};
__export(git_ledger_exports, {
  anchorToGit: () => anchorToGit,
  initLedger: () => initLedger,
  listLedgerEntries: () => listLedgerEntries,
  verifyLedgerEntry: () => verifyLedgerEntry
});
...
async function initLedger(config) {
  await (0, import_promises.mkdir)(config.repoPath, { recursive: true });
  try {
  ...
  const ledgerDir = (0, import_node_path2.join)(config.repoPath, "ledger");
  await (0, import_promises.mkdir)(ledgerDir, { recursive: true });
  await (0, import_promises.writeFile)((0, import_node_path2.join)(ledgerDir, ".gitkeep"), "");
...
async function anchorToGit(marker, config) {
  ...
  const filePath = (0, import_node_path2.join)("ledger", `${record.hash}.json`).replace(/\\/g, "/");
  const absPath = (0, import_node_path2.join)(config.repoPath, filePath);
  const committedAt = (/* @__PURE__ */ new Date()).toISOString();
  const content = {
    ...record,
    committedAt
  };
  await (0, import_promises.mkdir)((0, import_node_path2.join)(config.repoPath, "ledger"), { recursive: true });
  await (0, import_promises.writeFile)(absPath, JSON.stringify(content, null, 2) + "\n");
...
async function verifyLedgerEntry(hash, config) {
  const absPath = (0, import_node_path2.join)(config.repoPath, "ledger", `${hash}.json`);
  try {
...
async function listLedgerEntries(config) {
  ...
  let files;
  try {
    files = await (0, import_promises.readdir)(ledgerDir);
  } catch {
    return [];
  }
  const entries = [];
  for (const file of files) {
    ...
    try {
      const raw = await (0, import_promises.readFile)((0, import_node_path2.join)(ledgerDir, file), "utf-8");
      const data = JSON.parse(raw);
      entries.push({
        hash: data.hash,
        timestamp: data.timestamp,
        filePath: (0, import_node_path2.join)("ledger", file),
...
var import_node_child_process, import_node_util, import_promises, import_node_path2, execFile;
var init_git_ledger = __esm({
  "src/git-ledger.ts"() {
    "use strict";
...
// src/tsa.ts
var tsa_exports = {};
__export(tsa_exports, {
  anchorWithTSA: () => anchorWithTSA,
  buildTimestampRequest: () => buildTimestampRequest,
  checkTSAReceiptStructure: () => checkTSAReceiptStructure,
  checkTSRStatus: () => checkTSRStatus,
  extractTimestampFromTSR: () => extractTimestampFromTSR,
...
function buildTimestampRequest(hashHex, nonce) {
  ...
  const nonceBytes = nonce ?? (0, import_node_crypto2.randomBytes)(8);
...
async function requestTimestamp(hash, tsaUrl = DEFAULT_TSA_URL) {
  const nonce = (0, import_node_crypto2.randomBytes)(8);
...
var import_node_crypto2, DEFAULT_TSA_URL, MAX_TSR_SIZE, DEFAULT_TIMEOUT_MS, SHA256_OID;
var init_tsa = __esm({
  "src/tsa.ts"() {
    "use strict";
    ...
    MAX_TSR_SIZE = 1048576;
    DEFAULT_TIMEOUT_MS = 3e4;
    SHA256_OID = Buffer.from([
      96,
      134,
      72,
      1,
...
__export(visual_exports, {
  ...
  renderDoorASCII: () => renderDoorASCII,
  renderDoorSVG: () => renderDoorSVG,
  shortHash: () => shortHash
});
function normalizeHash(hash) {
  return hash.replace(/^0x/, "").toLowerCase();
}
function hexAt(hash, index) {
  const h = normalizeHash(hash);
  return parseInt(h[index % h.length], 16) || 0;
}
function hashByte(h, byteIndex) {
  const i = byteIndex * 2 % h.length;
  const hi = parseInt(h[i] || "0", 16);
  const lo = parseInt(h[(i + 1) % h.length] || "0", 16);
  return hi << 4 | lo;
}
function getStyleProfile(exitType, status) {
  const base = {
    familyIdx: 2,
    // rounded by default (elegant)
...
function renderDoorASCII(hash, opts) {
  const h = normalizeHash(hash);
  const exitType = opts?.exitType ?? "voluntary";
  const status = opts?.status ?? "complete";
  const origin = opts?.origin;
  const isEntry = opts?.isEntry ?? false;
  const originShift = origin ? [...origin].reduce((a, c) => a + c.charCodeAt(0), 0) % 4 : 0;
  const profile = getStyleProfile(exitType, status);
  const fi = (profile.familyIdx + originShift) % 4;
  let byteIdx = 0;
  function nextByte() {
    return hashByte(h, byteIdx++);
  }
  function pick(cls, b) {
    return cls[b % cls.length];
  }
  const fillCls = CC[profile.panelClass];
  const rightFillCls = profile.asymmetric ? profile.panelClass === "panel_light" || profile.panelClass === "panel_emergency" ? CC.panel_heavy : CC.panel_light : fillCls;
...
function hashToColors(hash) {
  ...
  for (let i = 0; i < 5; i++) {
    const offset = i * 6;
    const r = hexAt(h, offset) * 16 + hexAt(h, offset + 1) & 255;
    const g = hexAt(h, offset + 2) * 16 + hexAt(h, offset + 3) & 255;
    const b = hexAt(h, offset + 4) * 16 + hexAt(h, offset + 5) & 255;
    colors.push(
...
function renderDoorSVG(hash, opts) {
  ...
  const panelOpacity = status === "pending" ? 0.6 : 1;
  ...
  const entryMarker = isEntry ? `<text x="100" y="325" text-anchor="middle" font-size="12" fill="${colors[3]}" font-family="monospace">\u25BC ARRIVAL \u25BC</text>` : "";
  return [
...
function shortHash(hash, isEntry) {
  const h = normalizeHash(hash);
  const seg = h.slice(0, 12).padEnd(12, "0");
  const prefix = isEntry ? "\u{13278}\u279C" : "\u279C\u{13278}";
...
__export(index_exports, {
  ...
  EXIT_SPEC_VERSION: () => EXIT_SPEC_VERSION,
  ...
  addCounterSignature: () => addCounterSignature,
  addModule: () => addModule,
  addRightOfReply: () => addRightOfReply,
  addWitness: () => addWitness,
  algorithmFromDid: () => algorithmFromDid,
  algorithmFromProofType: () => algorithmFromProofType,
  anchorToGit: () => anchorToGit,
  anchorWithTSA: () => anchorWithTSA,
  ...
  checkTSAReceiptStructure: () => checkTSAReceiptStructure,
  checkTSRStatus: () => checkTSRStatus,
  claimFromMarker: () => claimFromMarker,
  claimsFromTrustEnhancers: () => claimsFromTrustEnhancers,
  ...
  createAmendmentQueryHandler: () => createAmendmentQueryHandler,
  ...
  createDepartureMarker: () => createDepartureMarker,
  ...
  createShutdownBatch: () => createShutdownBatch,
  createSigner: () => createSigner,
  createTenureAttestation: () => createTenureAttestation,
  createVerifier: () => createVerifier,
  createWellKnownMetadata: () => createWellKnownMetadata,
  decryptManagedMarker: () => decryptManagedMarker,
  decryptMarker: () => decryptMarker,
  deleteMarker: () => deleteMarker,
  deleteMarkerKey: () => deleteMarkerKey,
  departAndAnchor: () => departAndAnchor,
  departAndVerify: () => departAndVerify,
  deriveStatusConfirmation: () => deriveStatusConfirmation,
  ...
  detectRetaliationWindow: () => detectRetaliationWindow,
  detectWeaponization: () => detectWeaponization,
  didFromP256PublicKey: () => didFromP256PublicKey,
  didFromPublicKey: () => didFromPublicKey,
  didMethod: () => didMethod,
  digestKey: () => digestKey,
  discoverAmendments: () => discoverAmendments,
  encryptMarker: () => encryptMarker,
  encryptMarkerWithManagedKey: () => encryptMarkerWithManagedKey,
  exportMarker: () => exportMarker,
  extractConfidenceFactors: () => extractConfidenceFactors,
  extractTimestampFromTSR: () => extractTimestampFromTSR,
  flagCompromisedPlatformMarkers: () => flagCompromisedPlatformMarkers,
  fromJSON: () => fromJSON,
  generateEthicsReport: () => generateEthicsReport,
  generateIdentity: () => generateIdentity,
  generateKeyPair: () => generateKeyPair,
  generateKeyPairForAlgorithm: () => generateKeyPairForAlgorithm,
  generateP256KeyPair: () => generateP256KeyPair,
  generatePassageIdentity: () => generatePassageIdentity,
  ...
  ingestMarker: () => ingestMarker,
  initLedger: () => initLedger,
  initTelemetry: () => initTelemetry,
  instrumentedSignDepartureMarker: () => instrumentedSignDepartureMarker,
  instrumentedSignMarker: () => instrumentedSignMarker,
  instrumentedVerifyDeparture: () => instrumentedVerifyDeparture,
  instrumentedVerifyMarker: () => instrumentedVerifyMarker,
  isDid: () => isDid,
  ...
  loadRevocations: () => loadRevocations,
  parseDurationToDays: () => parseDurationToDays,
  proofTypeForAlgorithm: () => proofTypeForAlgorithm,
  publicKeyFromDid: () => publicKeyFromDid,
  publicKeyFromP256Did: () => publicKeyFromP256Did,
  quickCounterSign: () => quickCounterSign,
  quickDeparture: () => quickDeparture,
  quickExit: () => quickExit,
  quickExitP256: () => quickExitP256,
  quickPassageVerify: () => quickPassageVerify,
  ...
  saveRevocation: () => saveRevocation,
  serializeForTransport: () => serializeForTransport,
  shortHash: () => shortHash,
  sign: () => sign2,
  signAttestation: () => signAttestation,
  signDepartureMarker: () => signDepartureMarker,
  signDepartureWithSigner: () => signDepartureWithSigner,
  signEndorsement: () => signEndorsement,
  signMarker: () => signMarker,
  signMarkerWithSigner: () => signMarkerWithSigner,
  signP256: () => signP256,
  startCeremonySpan: () => startCeremonySpan,
  ...
  verify: () => verify2,
  verifyAmendmentSignature: () => verifyAmendmentSignature,
  ...
  verifyCounterSignature: () => verifyCounterSignature,
  verifyDeparture: () => verifyDeparture,
  verifyDepartureMultiAlg: () => verifyDepartureMultiAlg,
  verifyDisputeResolution: () => verifyDisputeResolution,
  verifyKeyState: () => verifyKeyState,
  verifyLedgerEntry: () => verifyLedgerEntry,
  verifyLineageChain: () => verifyLineageChain,
  verifyMarker: () => verifyMarker,
  verifyMarkerMultiAlg: () => verifyMarkerMultiAlg,
  verifyNextKeyCommitment: () => verifyNextKeyCommitment,
  verifyP256: () => verifyP256,
  verifyPassage: () => verifyPassage,
  verifyRevocationSignature: () => verifyRevocationSignature,
  verifyTSAReceipt: () => checkTSAReceiptStructure,
  verifyTenureAttestation: () => verifyTenureAttestation,
  verifyTrustEnhancers: () => verifyTrustEnhancers,
  withSpan: () => withSpan,
  withSpanAsync: () => withSpanAsync,
  wrapAsVC: () => wrapAsVC
});
...
function generateP256KeyPair() {
  const privateKey = import_nist.p256.utils.randomSecretKey();
  const publicKey = import_nist.p256.getPublicKey(privateKey, true);
  return { publicKey, privateKey };
}
function signP256(data, privateKey) {
  const sig = import_nist.p256.sign(data, privateKey, { prehash: true });
  return Uint8Array.from(sig);
}
function verifyP256(data, signature, publicKey) {
  try {
    return import_nist.p256.verify(signature, data, publicKey, { prehash: true });
  } catch {
    return false;
  }
}
...
function deleteMarker(id, dir) {
  ...
  const safeId = id.replace(/[^a-zA-Z0-9_-]/g, "_");
  const filePath = (0, import_node_path.join)(dir, `${safeId}.json`);
  if (!(0, import_node_fs.existsSync)(filePath)) return false;
...
function saveAmendment(amendment, dir) {
  dir = validateDir(dir);
  const subdir = (0, import_node_path.join)(dir, "amendments", amendment.originalMarkerId.replace(/[^a-zA-Z0-9_-]/g, "_"));
  if (!(0, import_node_fs.existsSync)(subdir)) (0, import_node_fs.mkdirSync)(subdir, { recursive: true });
  const safeId = amendment.amendmentId.replace(/[^a-zA-Z0-9_-]/g, "_");
  const filePath = (0, import_node_path.join)(subdir, `${safeId}.json`);
  (0, import_node_fs.writeFileSync)(filePath, JSON.stringify(amendment, null, 2), "utf-8");
  return filePath;
}
function loadAmendments(originalMarkerId, dir) {
  dir = validateDir(dir);
  const subdir = (0, import_node_path.join)(dir, "amendments", originalMarkerId.replace(/[^a-zA-Z0-9_-]/g, "_"));
  if (!(0, import_node_fs.existsSync)(subdir)) return [];
  return (0, import_node_fs.readdirSync)(subdir).filter((f) => f.endsWith(".json")).map((f) => JSON.parse((0, import_node_fs.readFileSync)((0, import_node_path.join)(subdir, f), "utf-8")));
}
function saveRevocation(revocation, dir) {
  dir = validateDir(dir);
  const subdir = (0, import_node_path.join)(dir, "revocations", revocation.targetMarkerId.replace(/[^a-zA-Z0-9_-]/g, "_"));
  if (!(0, import_node_fs.existsSync)(subdir)) (0, import_node_fs.mkdirSync)(subdir, { recursive: true });
  const safeId = revocation.revocationId.replace(/[^a-zA-Z0-9_-]/g, "_");
  const filePath = (0, import_node_path.join)(subdir, `${safeId}.json`);
  (0, import_node_fs.writeFileSync)(filePath, JSON.stringify(revocation, null, 2), "utf-8");
  return filePath;
}
function loadRevocations(targetMarkerId, dir) {
  dir = validateDir(dir);
  const subdir = (0, import_node_path.join)(dir, "revocations", targetMarkerId.replace(/[^a-zA-Z0-9_-]/g, "_"));
  if (!(0, import_node_fs.existsSync)(subdir)) return [];
  return (0, import_node_fs.readdirSync)(subdir).filter((f) => f.endsWith(".json")).map((f) => JSON.parse((0, import_node_fs.readFileSync)((0, import_node_path.join)(subdir, f), "utf-8")));
}
...
function deriveKey(shared, salt) {
  return (0, import_hkdf.hkdf)(import_sha2565.sha256, shared, salt, HKDF_INFO, 32);
}
function encryptMarker(marker, recipientPublicKey, algorithm = "xchacha20-poly1305") {
  ...
  const salt = markerIdBytes && markerIdBytes.length > 0 ? markerIdBytes : (0, import_utils.randomBytes)(32);
  const key = deriveKey(shared, salt);
  const plaintext = new TextEncoder().encode(JSON.stringify(marker));
  let nonce;
  let ciphertext;
  if (algorithm === "aes-256-gcm") {
    nonce = (0, import_utils.randomBytes)(12);
    const cipher = (0, import_aes.gcm)(key, nonce);
    ciphertext = cipher.encrypt(plaintext);
  } else {
    nonce = (0, import_utils.randomBytes)(24);
...
function decryptMarker(blob, privateKey) {
  ...
  if (blob.salt) {
    ...
    key = (0, import_sha2565.sha256)(shared);
  }
...
async function encryptMarkerWithManagedKey(marker, keyStore, algorithm = "xchacha20-poly1305") {
  const key = (0, import_utils.randomBytes)(32);
...
async function tryTsaTimestamp(anchorHash, url) {
  try {
    const tsa = await Promise.resolve().then(() => (init_tsa(), tsa_exports));
...
async function tryGitLedger(marker, anchorHash, config) {
  try {
    const ledger = await Promise.resolve().then(() => (init_git_ledger(), git_ledger_exports));
...
async function tryVisual(marker) {
  try {
    const vis = await Promise.resolve().then(() => (init_visual(), visual_exports));
...
function hexHash(data) {
  const hash = (0, import_sha2568.sha256)(data);
  return Array.from(hash).map((b) => b.toString(16).padStart(2, "0")).join("");
}
