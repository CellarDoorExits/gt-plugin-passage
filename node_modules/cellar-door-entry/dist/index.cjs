...
__export(index_exports, {
  AdmissionEventEmitter: () => AdmissionEventEmitter,
  CAUTIOUS: () => CAUTIOUS,
  EMERGENCY_ONLY: () => EMERGENCY_ONLY,
  ENTRY_CONTEXT_V1: () => ENTRY_CONTEXT_V1,
  InMemoryClaimStore: () => InMemoryClaimStore,
  LOCKDOWN: () => LOCKDOWN,
  MAX_MARKER_SIZE: () => MAX_MARKER_SIZE,
  OPEN_DOOR: () => OPEN_DOOR,
  OPEN_DOOR_V2: () => OPEN_DOOR_V2,
  PERMISSIVE: () => PERMISSIVE,
  PolicyBuilder: () => PolicyBuilder,
  QUARANTINE_UNKNOWN: () => QUARANTINE_UNKNOWN,
  REQUIRE_MUTUAL: () => REQUIRE_MUTUAL,
  REQUIRE_MUTUAL_WITH_ONRAMP: () => REQUIRE_MUTUAL_WITH_ONRAMP,
  STRICT: () => STRICT,
  SqliteClaimStore: () => SqliteClaimStore,
  admit: () => admit,
  bulkMint: () => bulkMint,
  canonicalize: () => canonicalize,
  computeArrivalId: () => computeArrivalId,
  createArrivalMarker: () => createArrivalMarker,
  createConfirmationRule: () => createConfirmationRule,
  createCustomRule: () => createCustomRule,
  createDisputeRule: () => createDisputeRule,
  createMintingRule: () => createMintingRule,
  createOriginRule: () => createOriginRule,
  createPolicy: () => createPolicy,
  createProbationaryArrival: () => createProbationaryArrival,
  createRestrictedScope: () => createRestrictedScope,
  createRevocationMarker: () => createRevocationMarker,
  createTrustLevelRule: () => createTrustLevelRule,
  evaluateAdmission: () => evaluateAdmission,
  formatCondition: () => formatCondition,
  isAdmissionStore: () => isAdmissionStore,
  isProbationComplete: () => isProbationComplete,
  isRevoked: () => isRevoked,
  ...
  quickEntryP256: () => quickEntryP256,
  resolveConflict: () => resolveConflict,
  scopeFromExitMarker: () => scopeFromExitMarker,
  signArrivalMarker: () => signArrivalMarker,
  validateArrivalMarker: () => validateArrivalMarker,
  verifyArrivalMarker: () => verifyArrivalMarker,
  verifyContinuity: () => verifyContinuity,
  verifyDeparture: () => verifyDeparture,
  verifyDepartureJSON: () => verifyDepartureJSON,
  verifyRevocationMarker: () => verifyRevocationMarker,
...
function verifyDeparture(exitMarker) {
  const result = (0, import_cellar_door_exit.verifyMarker)(exitMarker);
  return { valid: result.valid, errors: [...result.errors] };
}
function verifyDepartureJSON(exitMarkerJson) {
  try {
    const marker = (0, import_cellar_door_exit.fromJSON)(exitMarkerJson);
...
function signArrivalMarker(marker, privateKey, publicKey, algorithm = "Ed25519") {
  ...
  let proofType;
  if (algorithm === "P-256") {
    did = (0, import_cellar_door_exit2.didFromP256PublicKey)(publicKey);
    const sig = (0, import_cellar_door_exit2.signP256)(data, privateKey);
    signature = new Uint8Array(sig);
    proofType = "EcdsaP256Signature2019";
  } else {
    did = (0, import_cellar_door_exit2.didFromPublicKey)(publicKey);
    signature = (0, import_cellar_door_exit2.sign)(data, privateKey);
...
function verifyArrivalMarker(marker) {
  ...
  try {
    ...
    const alg = (0, import_cellar_door_exit2.algorithmFromDid)(did);
    const publicKey = alg === "P-256" ? (0, import_cellar_door_exit2.publicKeyFromP256Did)(did) : (0, import_cellar_door_exit2.publicKeyFromDid)(did);
    const { proof: _proof, ...rest } = marker;
    const canonical = canonicalize(rest);
    const data = new TextEncoder().encode(DOMAIN_PREFIX + canonical);
    ...
    const valid = alg === "P-256" ? (0, import_cellar_door_exit2.verifyP256)(data, signature, publicKey) : (0, import_cellar_door_exit2.verify)(data, signature, publicKey);
    if (!valid) {
...
function verifyContinuity(exitMarker, arrivalMarker) {
  ...
  const exitVerification = (0, import_cellar_door_exit3.verifyMarker)(exitMarker);
...
// src/convenience.ts
var import_cellar_door_exit8 = require("cellar-door-exit");

// src/admit.ts
var import_node_crypto = require("crypto");
var import_cellar_door_exit7 = require("cellar-door-exit");

// src/policy-builder.ts
var import_cellar_door_exit6 = require("cellar-door-exit");
...
var LEVEL_ORDER = [
  import_cellar_door_exit4.StatusConfirmation.SelfOnly,
  import_cellar_door_exit4.StatusConfirmation.OriginOnly,
  import_cellar_door_exit4.StatusConfirmation.Mutual,
  import_cellar_door_exit4.StatusConfirmation.Witnessed
];
...
function createConfirmationRule(config) {
  return {
    ...
    evaluate(marker, ctx) {
      ...
      const actual = (0, import_cellar_door_exit4.deriveStatusConfirmation)(marker);
...
var import_cellar_door_exit5 = require("cellar-door-exit");
function createTrustLevelRule(config) {
  return {
    ...
    evaluate(marker, ctx) {
      ...
      const level = (0, import_cellar_door_exit5.deriveStatusConfirmation)(marker);
      if (level !== import_cellar_door_exit5.StatusConfirmation.SelfOnly) {
...
var PolicyBuilder = class _PolicyBuilder {
  ...
  _resolveConfirmation(level) {
    const map = {
      self_only: import_cellar_door_exit6.StatusConfirmation.SelfOnly,
      origin_only: import_cellar_door_exit6.StatusConfirmation.OriginOnly,
      mutual: import_cellar_door_exit6.StatusConfirmation.Mutual,
      witnessed: import_cellar_door_exit6.StatusConfirmation.Witnessed
    };
    return map[level] ?? import_cellar_door_exit6.StatusConfirmation.SelfOnly;
  }
...
function generateRecordId() {
  return `admission-${(0, import_node_crypto.randomUUID)()}`;
}
...
async function admit(exitMarkerInput, opts = {}) {
  ...
  const pendingClaimId = `pending-${(0, import_node_crypto.randomUUID)()}`;
  ...
  if (exitMarker) {
    ...
    if (exitMarker.dispute?.counterpartyAcks?.length) {
      ...
      const verifiedAcks = exitMarker.dispute.counterpartyAcks.filter((ack, i) => {
        try {
          const algo = (0, import_cellar_door_exit7.algorithmFromDid)(ack.verificationMethod);
          const pubKey = algo === "P-256" ? (0, import_cellar_door_exit7.publicKeyFromP256Did)(ack.verificationMethod) : (0, import_cellar_door_exit7.publicKeyFromDid)(ack.verificationMethod);
          const result = (0, import_cellar_door_exit7.verifyCounterSignature)(markerForVerify, pubKey, i);
          return result.valid;
        } catch {
          return false;
        }
  ...
  if (!policyResult.admitted) {
    const record2 = {
      ...
      subjectDid: exitMarker?.subject ?? "unknown",
      originDid: exitMarker?.origin,
      confirmationLevel: exitMarker ? (0, import_cellar_door_exit7.deriveStatusConfirmation)(exitMarker) : void 0,
      policyApplied: policy.name,
      policyVersion: computePolicyVersion(policy),
      admitted: false,
  ...
  if (exitMarker && opts.platformIdentity && opts.counterSign !== false) {
    counterSignedExit = (0, import_cellar_door_exit7.addCounterSignature)(
      exitMarker,
  ...
  if (exitMarker) {
    ...
    arrivalMarker = {
      ...
      id: `urn:entry:minted-${(0, import_node_crypto.randomUUID)()}`,
...
function quickEntry(exitMarkerJson, destination, opts) {
  ...
  const exitMarker = (0, import_cellar_door_exit8.fromJSON)(exitMarkerJson);
  ...
  let privateKey;
  if (algorithm === "P-256") {
    const kp = (0, import_cellar_door_exit8.generateP256KeyPair)();
    publicKey = kp.publicKey;
    privateKey = kp.privateKey;
  } else {
    const kp = (0, import_cellar_door_exit8.generateKeyPair)();
    publicKey = kp.publicKey;
    privateKey = kp.privateKey;
  }
...
async function quickAdmit(exitMarker, opts) {
  ...
  if (typeof exitMarker === "string") {
    marker = (0, import_cellar_door_exit8.fromJSON)(exitMarker);
  } else if (exitMarker) {
    marker = exitMarker;
  }
...
// src/revocation.ts
var import_cellar_door_exit9 = require("cellar-door-exit");
...
function createRevocationMarker(arrivalMarker, reason, privateKey, publicKey, algorithm = "Ed25519") {
  const did = algorithm === "P-256" ? (0, import_cellar_door_exit9.didFromP256PublicKey)(publicKey) : (0, import_cellar_door_exit9.didFromPublicKey)(publicKey);
  ...
  const signature = algorithm === "P-256" ? (0, import_cellar_door_exit9.signP256)(data, privateKey) : (0, import_cellar_door_exit9.sign)(data, privateKey);
  const proofValue = btoa(String.fromCharCode(...signature));
...
function verifyRevocationMarker(marker, arrivalMarker) {
  ...
  try {
    const did = marker.proof.verificationMethod;
    const alg = (0, import_cellar_door_exit9.algorithmFromDid)(did);
    const publicKey = alg === "P-256" ? (0, import_cellar_door_exit9.publicKeyFromP256Did)(did) : (0, import_cellar_door_exit9.publicKeyFromDid)(did);
    const { proof: _proof, ...rest } = marker;
    const canonical = canonicalize(rest);
    const data = new TextEncoder().encode(DOMAIN_PREFIX2 + canonical);
    const sigStr = atob(marker.proof.proofValue);
    const signature = new Uint8Array(sigStr.length);
    for (let i = 0; i < sigStr.length; i++) signature[i] = sigStr.charCodeAt(i);
    const valid = alg === "P-256" ? (0, import_cellar_door_exit9.verifyP256)(data, signature, publicKey) : (0, import_cellar_door_exit9.verify)(data, signature, publicKey);
    if (!valid) {
...
// src/transfer.ts
var import_cellar_door_exit10 = require("cellar-door-exit");
function verifyTransfer(exitMarker, arrivalMarker) {
  const errors = [];
  const exitResult = (0, import_cellar_door_exit10.verifyMarker)(exitMarker);
  if (!exitResult.valid) {
