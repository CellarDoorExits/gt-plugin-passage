...
__export(index_exports, {
  GT_CAUTIOUS: () => GT_CAUTIOUS,
  GT_LOCKDOWN: () => GT_LOCKDOWN,
  GT_OPEN_DOOR: () => GT_OPEN_DOOR,
  GT_STANDARD: () => GT_STANDARD,
  MAX_TAGS: () => MAX_TAGS,
  MAX_TAG_LENGTH: () => MAX_TAG_LENGTH,
  PassageError: () => PassageError,
  buildOrigin: () => buildOrigin,
  createTrustedTownsPolicy: () => createTrustedTownsPolicy,
  crewTransfer: () => crewTransfer,
  extractGasTownContext: () => extractGasTownContext,
  fromBeadAttachment: () => fromBeadAttachment,
  getPassportChain: () => getPassportChain,
  getPassportChainFromStore: () => getPassportChainFromStore,
  getPolicy: () => getPolicy,
  mapReasonToExitType: () => mapReasonToExitType,
  markerToPassportEntry: () => markerToPassportEntry,
  polecatDepart: () => polecatDepart,
  toBeadAttachment: () => toBeadAttachment,
  townEntry: () => townEntry,
...
// src/bead.ts
var import_cellar_door_exit = require("cellar-door-exit");
...
function toBeadAttachment(marker) {
  return {
    ...
    payload: (0, import_cellar_door_exit.toJSON)(marker)
  };
}
function fromBeadAttachment(attachment) {
  ...
  const marker = (0, import_cellar_door_exit.fromJSON)(attachment.payload);
...
function mapReasonToExitType(reason, _mergeResult) {
  switch (reason) {
    case "completed":
    case "manual":
      return import_cellar_door_exit2.ExitType.Voluntary;
    case "witness-killed":
    case "timeout":
      return import_cellar_door_exit2.ExitType.Directed;
    case "crashed":
      return import_cellar_door_exit2.ExitType.Emergency;
    default: {
...
async function generateTransferToken(marker) {
  const canonical = (0, import_cellar_door_exit2.canonicalize)(marker);
...
async function polecatDepart(options) {
  ...
  let marker = (0, import_cellar_door_exit2.createMarker)({
    subject: signer.did(),
    origin,
    exitType,
    emergencyJustification: exitType === import_cellar_door_exit2.ExitType.Emergency ? `Polecat ${polecatId} crashed during execution` : void 0
  });
  const tags = [
    `polecat:${polecatId}`,
    `rig:${rigName}`,
  ...
  validateTags(tags);
  marker = (0, import_cellar_door_exit2.addModule)(marker, "metadata", {
  ...
  if (hookRef || beadIds && beadIds.length > 0) {
    ...
    marker = (0, import_cellar_door_exit2.addModule)(marker, "stateSnapshot", {
      stateHash,
      stateLocation: hookRef ? `gastown://${townId}/hooks/${hookRef}` : void 0,
      obligations: beadIds?.map((id) => `bead:${id}`)
    });
  }
  const signed = await (0, import_cellar_door_exit2.signMarkerWithSigner)(marker, signer);
  const result = {
    marker: signed,
...
async function crewTransfer(options) {
  ...
  let marker = (0, import_cellar_door_exit2.createMarker)({
    subject: signer.did(),
    origin,
    exitType: import_cellar_door_exit2.ExitType.Voluntary
  });
  const tags = [
    `crew:${crewName}`,
    `source:${sourceRig}`,
  ...
  marker = (0, import_cellar_door_exit2.addModule)(marker, "lineage", {
...
var GT_LOCKDOWN = (0, import_cellar_door_entry.createPolicy)("gt-lockdown").requireVerifiedDeparture().conflictResolution("deny-overrides").custom("reject-all", () => ({
  admitted: false,
  reason: "Town is in lockdown mode -- no cross-town agents accepted"
})).build();
var GT_CAUTIOUS = (0, import_cellar_door_entry.createPolicy)("gt-cautious").requireVerifiedDeparture().onSelfOnly("probation", { duration: "7d" }).onDisputed("quarantine", { maxDuration: "7d", reviewRequired: true }).conflictResolution("deny-overrides").build();
var GT_STANDARD = (0, import_cellar_door_entry.createPolicy)("gt-standard").requireVerifiedDeparture().maxAge("24h").conflictResolution("deny-overrides").build();
function createTrustedTownsPolicy(trustedTownIds) {
  const origins = trustedTownIds.map((id) => `gastown://${id}`);
  return (0, import_cellar_door_entry.createPolicy)("gt-trusted-towns").requireVerifiedDeparture().allowOrigins(origins).conflictResolution("deny-overrides").build();
}
var GT_OPEN_DOOR = (0, import_cellar_door_entry.createPolicy)("gt-open-door").requireVerifiedDeparture().defaultDecision("admit").conflictResolution("permit-overrides").build();
var PRESET_MAP = {
...
async function townEntry(options) {
  ...
  const result = await (0, import_cellar_door_entry.admit)(marker, {
    policy: builtPolicy,
    store,
    destination
  });
...
function getPassportChain(agentId, markers, options) {
  ...
  for (const marker of markers) {
    ...
    if (isMatch) {
      if (shouldVerify) {
        const verification = (0, import_cellar_door_exit3.verifyMarker)(marker);
